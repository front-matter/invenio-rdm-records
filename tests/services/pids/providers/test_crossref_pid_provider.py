...
def test_log_error_msg(app, mocker):
    ...
    def _test(mock_fn, err, expected_msg, expected_msg_args=None):
        CrossrefPIDProvider._log_errors(err)
    ...
    _log_fn = (
        "invenio_rdm_records.services.pids.providers.crossref.current_app.logger.error"
    )
    with app.app_context():
        _mocked_fn = mocker.patch(_log_fn)

        err = CrossrefError.factory(204, "")
        _test(_mocked_fn, err, "Crossref no content error")

        err = CrossrefError.factory(500, "down")
        _test(_mocked_fn, err, "Crossref internal server error")

        err = CrossrefError.factory(400, "{[")  # wrong json
        _test(_mocked_fn, err, "Unknown Crossref error")

        err = CrossrefError.factory(
        ...
        err = CrossrefError.factory(403, '{"errors": [{"title": "Unauthorized"}]}')
        _test(
            _mocked_fn,
            err,
            "Crossref error (field: %(field)s): %(reason)s",

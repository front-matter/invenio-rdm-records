...
"""Crossref based Schema for Invenio RDM Records."""
...
"""Allowed related identifier schemes for Crossref."""


def get_scheme_crossref(scheme, config_name, default=None):
    """Returns the crossref equivalent of a scheme."""
    config_item = current_app.config[config_name]
    return config_item.get(scheme, {}).get("crossref", default)
...
class CreatorSchema(PersonOrOrgSchema):
    """Creator schema for."""


class ContributorSchema(PersonOrOrgSchema):
    """Contributor schema."""
    ...
    def get_role(self, obj):
        ...
        props = get_vocabulary_props(
            "contributorsroles", ["props.crossref"], role["id"]
        )
        return props.get("crossref", "")
...
class CrossrefSchema(BaseSerializerSchema):
    """Crossref JSON Marshmallow Schema."""
    ...
    contributors = fields.List(
        fields.Nested(ContributorSchema), attribute="metadata.contributors"
    )
    ...
    schemaVersion = fields.Constant("http://crossref.org/schema/")
    ...
    def get_type(self, obj):
        ...
        props = get_vocabulary_props(
            "resourcetypes",
            ["props.crossref_general", "props.crossref_type"],
            resource_type_id,
        )
        return {
            "resourceTypeGeneral": props.get("crossref_general", "Other"),
            "resourceType": props.get("crossref_type", ""),
        }
    ...
    def _merge_main_and_additional(self, obj, field, default_type=None):
        ...
        for v in additional_values:
            ...
            if type_id:
                props = get_vocabulary_props(
                    f"{field}types", ["props.crossref"], type_id
                )
                if "crossref" in props:
                    item[f"{field}Type"] = props["crossref"]
    ...
    def get_dates(self, obj):
        ...
        for date in obj["metadata"].get("dates", []):
            ...
            if date_type_id == "updated":
                updated = True
            props = get_vocabulary_props("datetypes", ["props.crossref"], date_type_id)
            to_append = {
                "date": date["date"],
                "dateType": props.get("crossref", "Other"),
            }
    ...
    def get_language(self, obj):
        ...
        if languages:
            # Crossref supports only one language, so we take the first.
    ...
    def get_identifiers(self, obj):
        ...
        for id_ in identifiers:
            ...
            if id_scheme:
                # Crossref only accepts a DOI identifier that is the official
    ...
    def get_related_identifiers(self, obj):
        ...
        for rel_id in identifiers:
            ...
            props = get_vocabulary_props(
                "relationtypes", ["props.crossref"], relation_type_id
            )
